...
- **核心接口草案**：
  ...
  - `DELETE /categories/{id}`、`POST /categories/{id}/restore`：调用 NDR 软删除/恢复节点；结合 `GET /categories/tree?include_deleted=true` 和 `GET /categories/trash`（可选）提供回收站能力。
...
## 六、NDR 集成策略
- **请求封装**：集中管理 API Key、User ID、Request ID。通过中间件在服务端注入 `x-user-id`、`x-request-id`，前端只需在特殊场景透传；`x-api-key` 默认读取配置。
- **节点树模型**：
