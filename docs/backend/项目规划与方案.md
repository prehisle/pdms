...
- 数据模型：
  - 节点 Node：以 `path`/`parent_path` 组织树形结构，适合承载题库目录。
    - 创建根目录时使用虚拟根路径（`/`），前端仅提供目录名；后端根据名称生成唯一 `slug` 和完整 `path`。
...
- 假设：题目与目录之间的一对多关系通过 Relationship 表达；节点排序需求可通过维护 `metadata` 或新增 `position` 字段在业务层处理（NDR 当前未提供排序字段）。
...
1. **节点管理（MVP）**
   ...
   - 排序维护：在拖拽同级节点时，后端维护额外的排序字段（存于节点 `metadata.order`），并批量更新。
2. **题目与资料管理**
...
- **核心接口草案**：
  - `GET /categories/tree`：按需加载节点树，支持 `depth` 参数。
  - `POST /categories`、`PATCH /categories/{id}`、`DELETE /categories/{id}`、`POST /categories/{id}/restore`：`POST` 入参包含目录名与可选 `parent_id`，空则创建根目录。
  - `PATCH /categories/{id}/move`：处理拖拽，入参包含目标父节点、排序位置；内部调用 `PUT /api/v1/nodes/{id}` 并批量更新同级节点的排序元数据。
...
- **节点树模型**：
  - `NodeCreate` / `NodeUpdate` 需提供 `name`、`slug`、`parent_path`。拖拽移动节点时，需重新计算 `parent_path` 与 `path`，确保全树一致。
  - `List Children` 暂不返回排序信息，需业务层补充。
- **文档模型**：
...
## 八、迭代计划（建议）
0. **迭代零：架构骨架**
   - 搭建 Go 服务基础目录结构（`cmd/`, `internal/api`, `internal/service`, `internal/ndrclient`, `internal/cache` 等）与通用中间件、配置加载、路由框架。
   - 前端创建 React/Antd 项目骨架，完成路由、状态管理、API 封装的基础目录布局。
   - 输出 README 或技术说明，供审核与后续迭代复用。
1. **迭代一：目录 MVP**
...
## 九、风险与应对
- 节点排序缺乏原生支持：通过业务字段维护排序，必要时在 NDR 中增加扩展字段或引入辅助表。
